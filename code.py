...
def agg_values_ina_month(df,date_col,agg_col, agg):
    ...
    """
    df[date_col]=pd.to_datetime(df[date_col]).dt.strftime('%B')
    aggregated_value=pd.pivot_table(df,index=date_col,values=agg_col,aggfunc='mean')
...
d={'Temp (C)','mean'}
print(agg_values_ina_month(weather,'Date/Time','Temp (C)',d))
...
dic1={'Temp (C)':'mean','Dew Point Temp (C)':'mean','Rel Hum (%)':'mean','Wind Spd (km/h)':'mean','Visibility (km)':'mean','Stn Press (kPa)':'mean'}
mean_weather=group_values(weather,'Weather',dic1)
print(mean_weather)
